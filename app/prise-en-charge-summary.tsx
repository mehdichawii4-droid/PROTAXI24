...
export default function PriseEnChargeSummaryScreen() {
  const data = useLocalSearchParams();

  const passengers = String(data.passengers || '1');
  const bags = String(data.bags || '1');

  const priceClean = String(data.price || '0')
    .replace(' DA', '')
    .replace(/\s/g, '');
  ...
  const confirmRequest = () => {
    router.push({
      ...
      params: {
        service: data.service || 'Prise en charge',
        mode: data.mode || 'Chauffeur privé',
        airport: data.airport || data.destination || 'Destination à confirmer',
        address: data.address || data.departure || 'Départ à confirmer',
        date: data.date || 'Maintenant',
        time: data.time || 'Maintenant',
        passengers,
        bags,
        tripType:
          data.tripType === 'Aller-retour'
            ? 'retour'
            : 'simple',
        price: priceClean,
      },
...
>
          <View style={styles.heroOverlay}>
            ...
            <Text style={styles.title}>Résumé Prise en charge</Text>
            <Text style={styles.subtitle}>Chauffeur privé premium</Text>

            <View style={styles.statusBadge}>
              <MaterialCommunityIcons name="account-tie" size={28} color={goldLight} />
              <View style={{ flex: 1 }}>
        ...
        <SectionTitle icon="document-text-outline" title="Détails de la mission" />

        <View style={styles.card}>
          <InfoRow
            icon="car-sport-outline"
            label="Service"
            value={String(data.service || 'Prise en charge')}
          />

          <InfoRow
            icon="briefcase-outline"
            label="Déplacement"
            value={String(data.mode || 'Chauffeur privé')}
          />

          <InfoRow
            icon="location-outline"
            label="Départ"
            value={String(data.address || 'Départ à confirmer')}
          />

          <InfoRow
            icon="navigate-outline"
            label="Destination"
            value={String(data.airport || data.destination || 'Destination à confirmer')}
          />

          <InfoRow
            icon="swap-horizontal-outline"
            label="Type de trajet"
            value={String(data.tripType || 'Aller simple')}
          />
          ...
          <InfoRow
            icon="time-outline"
            label="Heure"
            value={String(data.time || 'Maintenant')}
          />
        </View>
